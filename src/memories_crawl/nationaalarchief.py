...
# The third element is the kantoor the range belongs to, so a fallback listing
# still carries the kantoor column.
...
def _fetch_inventory_entries(session: requests.Session) -> list[dict]:
    """Fetch the EAD XML and parse section 2.4 into inventory rows.
...
def _count_scans(session: requests.Session, invnr: int) -> int | None:
    ...
    try:
        ...
        return None
    return len(_extract_scans_from_viewer(resp.text))
...
def main(
    ...
    entries = _fetch_inventory_entries(session)
    print(
    ...
    for invnr in inv_numbers:
        ...
        print(f"{len(scans)} scans{listing.summary_suffix(len(scans))}")
        with open(done_file, "a") as f:
            f.write(key + "\n")
        time.sleep(1.0)
